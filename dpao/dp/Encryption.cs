...
namespace Ronlier
{
    ...
    public class Encryption
    {
        ...
        {
            byte[] b = DecryptToBytes(cipherTextBytes);
        ...
        public byte[] DecryptToBytes(byte[] cipherTextBytes)
        {
            ...
            if (saltLen > decryptedByteCount) return null;
        ...
        #region AES 加密静态方法
        /// <summary>
        /// 加密:将srcStr字符进行AES加密后返回加密字串,sKey可以随意,sIV必须为16位字符或数字组成.
        /// </summary>
        public static string AESEncrypt(string srcStr, string sKey, string sIV)
        {
            try
            {
                Encryption aes = new Encryption(sKey, sIV);
                string str = aes.Encrypt(srcStr);
                aes = null;
                return str;
            }
            catch (Exception ex)
        ...
        #region AES 解密静态方法
        /// <summary>
        /// 解密:将行AES加密后的字串EncryptStr进行解密,返回解密后的明文字串,sKey可以随意,sIV必须为16位字符或数字组成.
        /// </summary>
        public static string AESDecrypt(string EncryptStr, string sKey, string sIV)
        {
            ...
            {
                Encryption aes = new Encryption(sKey, sIV);
                string str = aes.Decrypt(EncryptStr);
                aes = null;
                return str;
            }
            catch (Exception ex)
        ...
        public static string MD5(string src)
        {
            ...
            byte[] bytes_md5_in = UTF8Encoding.Default.GetBytes(src);
        ...
        #region SHA1 加密方法
        /// <summary>
        /// SHA1 加密方法，参数src为明文字符
        /// </summary>
        public static string SHA1(string src)
        {
            SHA1 sha1 = new SHA1CryptoServiceProvider();
            byte[] bytes_sha1_in = UTF8Encoding.Default.GetBytes(src);
